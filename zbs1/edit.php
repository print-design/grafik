    ...
    <head>
        ...
        if($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['typography_edit_submit'])) {
            if($form_valid) {
                ...
                $sql = "update zbs set typographer_id=$typographer_id, organization_id=$organization_id, edition_id=$edition_id, length=$length, lamination_id=$lamination_id, coloring=$coloring, roller=$roller, manager_id=$manager_id where id=$id";
                ...
                if ($conn->query($sql) === true) {
                    ...
                    if(isset($_POST['to']) && $_POST['to'] != '')
                        $period['to'] = $_POST['to'];
                    header('Location: '.APPLICATION.'/zbs1/?'. http_build_query($period));
                }
                else {
        ...
        if(!isset($_GET['from']) || !isset($_GET['to']) || !isset($_GET['id'])) {
            header('Location: '.APPLICATION.'/zbs1/');
        }
        ...
        $conn = new mysqli(DATABASE_HOST, DATABASE_USER, DATABASE_PASSWORD, DATABASE_NAME);
        $sql = "select t.id, date_format(t.date, '%d.%m.%Y') date, t.shift, t.typographer_id, "
            ...
            <div class="row">
                <div class="col-12 col-md-6">
                    ...
                    <div class="d-flex justify-content-between mb-2">
                        <div class="p-1">
                            <h1>ZBS-1, редактирование рабочей смены</h1>
                        </div>
                        <div class="p-1">
                            <a href="<?=APPLICATION ?>/zbs1/?from=<?=$_GET['from'] ?>&to=<?=$_GET['to'] ?>" class="btn btn-outline-dark"><span class="font-awesome">&#xf0e2;</span>&nbsp;Отмена</a>
                        </div>
                    </div>
