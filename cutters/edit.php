    ...
    <head>
        ...
        if($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['typography_edit_submit'])) {
            if($form_valid) {
                ...
                $sql = "update cutters set cutter_id=$cutter_id, organization_id=$organization_id, edition_id=$edition_id, length=$length, lamination_id=$lamination_id, roller=$roller, manager_id=$manager_id, comment='$comment' where id=$id";
                ...
                if ($conn->query($sql) === true) {
                    ...
                    if(isset($_POST['to']) && $_POST['to'] != '')
                        $period['to'] = $_POST['to'];
                    header('Location: '.APPLICATION.'/cutters/?'. http_build_query($period));
                }
                else {
        ...
        if(!isset($_GET['from']) || !isset($_GET['to']) || !isset($_GET['id'])) {
            header('Location: '.APPLICATION.'/cutters/');
        }
        ...
        $conn = new mysqli(DATABASE_HOST, DATABASE_USER, DATABASE_PASSWORD, DATABASE_NAME);
        $sql = "select t.id, date_format(t.date, '%d.%m.%Y') date, t.shift, t.cutter_id, "
            ...
            <div class="row">
                <div class="col-12 col-md-6">
                    ...
                    <div class="d-flex justify-content-between mb-2">
                        <div class="p-1">
                            <h1>Резчики, редактирование рабочей смены</h1>
                        </div>
                        <div class="p-1">
                            <a href="<?=APPLICATION ?>/cutters/?from=<?=$_GET['from'] ?>&to=<?=$_GET['to'] ?>" class="btn btn-outline-dark"><span class="font-awesome">&#xf0e2;</span>&nbsp;Отмена</a>
                        </div>
                    </div>
                    ...
                    <form method="post">
                        ...
                        <div class="row">
                            <div class="col-12 col-md-6">
                                <div class="form-group">
                                    <label for="cutter_id">Резчик</label>
                                    <select id="cutter_id" name="cutter_id" class="form-control" required="required">
                                        <option value="">...</option>
                                        <?php
                                        $conn = new mysqli(DATABASE_HOST, DATABASE_USER, DATABASE_PASSWORD, DATABASE_NAME);
                                        if($conn->connect_error) {
                                        ...
                                        if ($result->num_rows > 0) {
                                            while ($row = $result->fetch_assoc()) {
                                                $selected = $_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['cutter_id']) && $_POST['cutter_id'] == $row['id'] ? " selected='selected'" : "";
                                                if($selected == '' && $cutter_id == $row['id']) $selected = " selected='selected'";
                                    ...
                                    <div class="invalid-feedback">Резчик обязательно</div>
                                </div>
                            </div>
                        </div>
                        <div class="row">
                        ...
                        <div class="form-group">
                            ...
                            <textarea rows="5" id="comment" name="comment" class="form-control"><?=$_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['comment']) ? htmlentities($_POST['comment']) : $comment ?></textarea>
                        </div>
                        <div class="form-group">
